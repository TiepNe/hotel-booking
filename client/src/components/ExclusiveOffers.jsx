...
const ExclusiveOffers = () => {
  return (
    ...
    pt-20 pb-30'>
      ...
      w-full'>
        <Title align='left' title='Ưu Đãi Độc Quyền' subTitle='Tận hưởng ưu đãi 
        giới hạn cùng gói dịch vụ đặc biệt, nâng tầm kỳ nghỉ của bạn và lưu giữ 
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12'>
        {exclusiveOffers.map((item)=>(
            ...
            image})`}}>
                ...
                text-gray-800 font-medium rounded-full'>Giảm {item.priceOff}%</p>
                <div>
                    <p className='text-2xl font-medium font-playfair'>{item.title}</p>
                    <p>{item.description}</p>
                    <p className='text-xs text-white/70 mt-3'>Áp dụng đến {item.expiryDate}</p>
                </div>
